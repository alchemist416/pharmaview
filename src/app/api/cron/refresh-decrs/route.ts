...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (establishments.length === 0) {
      return NextResponse.json({ error: 'No DECRS data fetched', refreshed: false });
    }
    ...
    if (error) {
      console.error('[cron/refresh-decrs] Supabase error:', error);
    ...
    console.error('[cron/refresh-decrs] Failed:', err);
    return NextResponse.json(
